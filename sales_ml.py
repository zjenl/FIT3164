...
try:
    hybrid_rnn = HybridRNN(cell_type='gru', state_sizes=[128, 64], seq_input_shape=(7, 1), current_price_shape=(1,))
    hybrid_rnn.build()
    ...
    rmse = np.sqrt(mse)
    print(f"cell_type='gru', state_sizes=[128, 64]: RMSE={rmse:.4f}")

    # Save the model to HDF5 file
    hybrid_rnn.model.save('FOODS_1_rnn_model.h5')
